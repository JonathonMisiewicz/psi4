...
extern void buf_ols_updt(double *a, double *c, double *norm, double *ovrlap, double *c1norm, int len);
...
**
*/
void CIWavefunction::mitrush_iter(CIvect &Hd, struct stringwr **alplist, struct stringwr **betlist, int nroots,
                                  double *evals, double conv_rms, double conv_e, double enuc, double edrc, int maxiter,
                                  int maxnvect) {
    ...
    Cvec.set(Parameters_->icore, maxnvect, 1, Parameters_->s_filenum, CIblks_);
    Sigma.set(Parameters_->icore, maxnvect, 1, Parameters_->s_filenum, CIblks_);
    ...
    olsen_update(Cvec, Sigma, Hd, E, E_est, &norm, &c1norm, &S, buffer1, buffer2, curr, last, "outfile", iter, alplist,
                 betlist);
    ...
    S *= norm;
    Cvec.buf_unlock();
    if (Parameters_->calc_ssq && Parameters_->icore == 1) Cvec.calc_ssq(buffer1, buffer2, alplist, betlist, 0);
    ...
    */
    ...
    while (true) {
       ...
       */
        ...
        if (iter == maxiter) outfile->Printf("Maximum number of iterations reached!\n");

        /* if the 2x2 matrix diagonalization can be done, take Mitrush Step */

        if (Parameters_->diag_method == METHOD_MITRUSHENKOV && diag_method == METHOD_MITRUSHENKOV && S < S_MAX &&
            ...
            */
            ...
            Cvec.buf_unlock();
            Sigma.buf_unlock();
            if (Parameters_->diag_method == METHOD_MITRUSHENKOV && diag_method == METHOD_MITRUSHENKOV) {
        ...
        */
        ...
        olsen_update(Cvec, Sigma, Hd, E, E_est, &norm, &c1norm, &S, buffer1, buffer2, curr, last, "outfile", iter,
                     alplist, betlist);
        ...
        Cvec.read(last, 0);
        S *= norm;
        if (Parameters_->precon >= PRECON_GEN_DAVIDSON && diag_method == METHOD_MITRUSHENKOV && S < S_MAX)
...
*/
void CIWavefunction::olsen_update(CIvect &C, CIvect &S, CIvect &Hd, double E, double E_est, double *norm,
                                  double *c1norm, double *ovrlap, double *buffer1, double *buffer2, int curr, int next,
                                  std::string out, int iter, struct stringwr **alplist, struct stringwr **betlist) {
    int buf;
    ...
    for (buf = 0; buf < C.buf_per_vect_; buf++) {
        ...
        S.buf_unlock();
        Hd.buf_lock(buffer2);
        if (Parameters_->hd_otf == FALSE)
            Hd.read(0, buf);
        else
            Hd.diag_mat_els_otf(alplist, betlist, CalcInfo_->onel_ints->pointer(), CalcInfo_->twoel_ints->pointer(),
                                CalcInfo_->edrc, CalcInfo_->num_alp_expl, CalcInfo_->num_bet_expl, CalcInfo_->nmo, buf,
                                Parameters_->hd_ave);
        ...
        */
        ...
        if (Parameters_->precon >= PRECON_GEN_DAVIDSON) C.h0block_buf_ols(&tmp1, &tmp2, &tmpnorm, E_est);
    ...
    *ovrlap = ox;
    if (normc1 <= 1.0E-13) {
        ...
        outfile->Printf(
            "This may cause numerical errors which would"
...
** Returns: none
*/
void CIWavefunction::olsen_iter_xy(CIvect &C, CIvect &S, CIvect &Hd, double *x, double *y, double *buffer1,
                                   double *buffer2, double E, int curvect, int L, double **alpha,
    ...
    *y = 0.0;
    ...
    for (buf = 0; buf < C.buf_per_vect_; buf++) {
        ...
        C.buf_lock(buffer1);
        C.read(curvect, buf);
        if (Parameters_->diag_method == METHOD_DAVIDSON_LIU_SEM) C.h0block_gather_vec(CI_VEC);
        if (Parameters_->hd_otf == FALSE)
            Hd.read(0, buf);
        else
            Hd.diag_mat_els_otf(alplist, betlist, CalcInfo_->onel_ints->pointer(), CalcInfo_->twoel_ints->pointer(),
        ...
        if (Parameters_->diag_method <= METHOD_MITRUSHENKOV) {
            ...
            for (i = 0; i < L; i++) {
                ...
                for (j = 0; j < H0block_->size; j++) sigma0b2[j] += alpha[i][curvect] * sigma0b1[j];
            }
        }
        if (C.buf_offdiag_[buf]) {
            *x += 2.0 * tx;
    ...
    Hd.buf_unlock();
    if (Parameters_->diag_method == METHOD_DAVIDSON_LIU_SEM) {
        for (j = 0; j < H0block_->size; j++) H0block_->s0b[j] = sigma0b2[j];
        free(sigma0b1);
        free(sigma0b2);
    }
}
...
**
*/
void CIWavefunction::mitrush_update(CIvect &C, CIvect &S, double norm, double acur, double alast, double *buffer1,
                                    double *buffer2, int curr, int next) {
    ...
    if (!Parameters_->Ms0)
        ...
        phase = ((int)Parameters_->S % 2) ? -1.0 : 1.0;
