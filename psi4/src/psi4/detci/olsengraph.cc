...
extern void print_ci_space(struct stringwr *strlist, int num_strings, int nirreps, int strtypes, int nel, int repl_otf);
extern void str_abs2rel(int absidx, int *relidx, int *listnum, struct olsen_graph *Graph);
...
void CIWavefunction::form_strings() {
    ...
    if (print_ >= 4) {
        for (irrep = 0, listnum = 0; irrep < nirreps; irrep++) {
            for (code = 0; code < ncodes; code++, listnum++) {
                ...
                print_ci_space(alplist_[irrep * ncodes + code], AlphaG_->sg[irrep][code].num_strings, nirreps, nlists,
                               AlphaG_->num_el_expl, Parameters_->repl_otf);
            }
        }
    }
    ...
    if (CalcInfo_->iopen && !(Parameters_->Ms0)) {
        ...
        olsengraph(BetaG_, CalcInfo_->num_ci_orbs, CalcInfo_->num_bet, CalcInfo_->nirreps, CalcInfo_->orbsym,
                   Parameters_->b_ras1_lvl, Parameters_->b_ras1_min, Parameters_->b_ras1_max, Parameters_->ras3_lvl,
                   Parameters_->b_ras3_max, CalcInfo_->num_drc_orbs, CalcInfo_->num_expl_cor_orbs,
                   Parameters_->ras4_lvl, Parameters_->b_ras4_max, Parameters_->b_ras3_max, Parameters_);

        if (print_ > 3) og_print(BetaG_);
        ...
        if (print_ >= 4) {
            for (irrep = 0; irrep < nirreps; irrep++) {
                for (code = 0; code < ncodes; code++) {
                    ...
                    print_ci_space(betlist_[irrep * ncodes + code], BetaG_->sg[irrep][code].num_strings, nirreps,
                                   nlists, BetaG_->num_el_expl, Parameters_->repl_otf);
                }
            }
        }
    }
    ...
    if (Parameters_->filter_guess) {
        str_abs2rel(Parameters_->filter_guess_Ia, &Parameters_->filter_guess_Iaridx, &Parameters_->filter_guess_Iac,
                    AlphaG_);
        str_abs2rel(Parameters_->filter_guess_Ib, &Parameters_->filter_guess_Ibridx, &Parameters_->filter_guess_Ibc,
                    BetaG_);
        str_abs2rel(Parameters_->filter_guess_Ja, &Parameters_->filter_guess_Jaridx, &Parameters_->filter_guess_Jac,
                    AlphaG_);
        str_abs2rel(Parameters_->filter_guess_Jb, &Parameters_->filter_guess_Jbridx, &Parameters_->filter_guess_Jbc,
                    BetaG_);
    }
    if (Parameters_->filter_zero_det) {
        str_abs2rel(Parameters_->filter_zero_det_Ia, &Parameters_->filter_zero_det_Iaridx,
                    &Parameters_->filter_zero_det_Iac, AlphaG_);
        str_abs2rel(Parameters_->filter_zero_det_Ib, &Parameters_->filter_zero_det_Ibridx,
                    &Parameters_->filter_zero_det_Ibc, BetaG_);
    }
    for (i = 0; i < Parameters_->follow_vec_num; i++) {
        str_abs2rel(Parameters_->follow_vec_Ia[i], &Parameters_->follow_vec_Iaridx[i], &Parameters_->follow_vec_Iac[i],
                    AlphaG_);
...
void olsengraph(struct olsen_graph *Graph, int ci_orbs, int num_el, int nirreps, int *orbsym, int ras1_lvl,
    ...
    Graph->decode = (int ***)malloc((ras1_max - ras1_min + 1) * sizeof(int **));
    for (i = 0; i < (ras1_max - ras1_min + 1); i++) {
        Graph->decode[i] = init_int_matrix(ras3_max + 1, ras4_max + 1);
    }

    encode_tmp = init_int_matrix(3, (ras1_max - ras1_min + 1) * (ras3_max + 1) * (ras4_max + 1));
        ...
        for (i = ras1_max; i >= ras1_min; i--) {
            for (j = 0; j <= ras3_max; j++) {
                for (k = 0; k <= ras4_max; k++) {
                    ...
                    if ((i + j + k <= num_el_expl) && (num_el_expl - i - j - k <= ras3_lvl - ras1_lvl - 1) &&
                        (j + k <= ras34_max) && (!(CIparams->r4s && k >= 2 && ras1_max - i > CIparams->ex_lvl))) {
    ...
    Graph->sg = (struct stringgraph **)malloc(nirreps * sizeof(struct stringgraph *));

    Graph->list_offset = init_int_array(nirreps * Graph->subgr_per_irrep);

    for (i = 0; i < nirreps; i++) {
        (Graph->sg)[i] = (struct stringgraph *)malloc(code * sizeof(struct stringgraph));

        for (j = 0; j < code; j++) {
            sgptr = (Graph->sg)[i] + j;
            sgptr->lvl = (struct level *)malloc((ci_orbs + 1) * sizeof(struct level));
                    ...
                    do {
                        ...
                        do {
                            ...
                            do {
                                ...
                                for (i = n1 - 1, j = num_expl_cor_orbs; i >= 0; i--) occs[j++] = array1[i];
                                for (i = n2 - 1; i >= 0; i--) occs[j++] = array2[i];
                                for (i = n3 - 1; i >= 0; i--) occs[j++] = array3[i];
        ...
        for (code = 0, offset = 0; code < ncodes; code++) {
            ...
            for (i = 0; i < norb; i++) {
                ...
                for (j = 0; j < (next->num_j); j++) {
                    (next->a)[j] = temp_a[j];
                    (next->b)[j] = temp_b[j];
                    for (s = 0; s < 2; s++) (next->kbar)[s][j] = temp_kbar[s][j];
                }
...
int og_calc_y(struct level *lvl, int ci_orbs) {
    ...
    for (i = 0; i < next->num_j; i++) num_strings += next->x[i];
    ...
    for (i = 0; i < ci_orbs; i++) {
        ...
        for (j = 0; j < (curr->num_j); j++) {
            ...
            if (ksij < 0)
                ...
                if (ksijbar >= 0) (curr->y)[j] = (curr->x)[ksijbar];
            }
        }
    }
            ...
            if (subgraph->num_strings) {
                ...
                for (k = 0; k < Graph->num_orb + 1; k++, curr++) {
                    for (l = 0; l < curr->num_j; l++) {
                        a = (curr->a)[l];
                        b = (curr->b)[l];
                        outfile->Printf("%7c%3d %3d %3d %3d %3d %3d %3d %3d %3d %3d\n", ' ', k, l + 1, a, b,
                                        (curr->k)[0][l], (curr->k)[1][l], (curr->kbar)[0][l], (curr->kbar)[1][l],
                                        (curr->x)[l], (curr->y)[l]);
                    }
                }
            }
...
void print_ci_space(struct stringwr *strlist, int num_strings, int nirreps, int strtypes, int nel, int repl_otf) {
    ...
    while (cnt != num_strings) {
        outfile->Printf("\nString %4d (", cnt++);
        for (i = 0; i < nel; i++) outfile->Printf("%2d ", (int)(strlist->occs)[i]);
        outfile->Printf(")\n");
